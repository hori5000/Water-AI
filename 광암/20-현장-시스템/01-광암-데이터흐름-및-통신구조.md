...
| Historian 실제 시계열 값 | **운영 POS11에서 조회 확인 / 장기간 Export 미인수** | `Runtime.dbo.History`에서 실제 `DateTime+vValue` 반환 확인. 단, 인수한 `Runtime/Holding/backup2` BAK에는 장기 값 본체가 없음 |
...
- 그러나 BAK 내부에는 AI 학습용 장기간 공정값 본체가 저장된 것으로 확인되지 않음
- **추가 현장 사진에서 POS11 `Runtime.dbo.History`로 실제 값 조회가 확인됨**. 따라서 실제 값은 운영 Historian에서 접근 가능하며, 현재 미완료 사항은 장기간 Export/인수다.
