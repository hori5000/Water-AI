...
또한 2024 HMI에는 `P52`, `P56`, `PLC3`가 추가 존재한다. 이들은 현재 13개 Device 목록에 없으므로 과거/보조/별도 Topic 가능성을 열어두고 PLC 대수로 바로 해석하지 않는다.
...
4. **`dde.cfg` Point Mapping 활용** — 이미 확보한 14,983개 GFENet Tag/Item을 PLC Address Dictionary와 대조
5. **기존 사전자료 선택 재수집** — `tagname.x`, `dhistcfg.ini`, `alarm.cfg`, `historian.txt`
6. **현재 DXP Application 확인** — `GFENet ↔ DeviceXPlorer` 관계 확정
7. **Historian Tag Export 확보** — InTouch/History ↔ Historian 연결
