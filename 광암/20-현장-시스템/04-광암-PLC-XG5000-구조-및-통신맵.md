...
> **주요 PCS가 192.9.211.x / 192.9.212.x 두 FEnet 경로를 사용한다. 과거 제어망 문서에서는 각각 LINE A / LINE B로 직접 명명돼 있다.**
...
Wonderware `DXPSV.dxp`에서 2026 현장 기준 직접 확인된 상위 수집 경로는 `192.9.211.x` 계열이다.

사전 제공 2024 InTouch `dde.cfg`에서는 별도로:
...
가 직접 확인됐다. 따라서 `212.x`가 실제 HMI 이중 Source에 사용된 흔적은 확인됐지만, 이를 모든 PLC의 Primary/Standby 구조로 일반화하지 않는다.
...
| P8 | 관리본관 계열 — 정확 역할 재확인 필요 | `192.9.211.18` |
...
### 6.1 사전 제공 2024 HMI와의 교차검증

2024 InTouch `dde.cfg`에서 `GFENet` Source의 주요 Topic은 다음 13개다.

```text
P1, P2, P4, P6, P7, P8, P21, P22, P23, P25, P26, P27, P30
```

이는 위 2026 DeviceXPlorer Device 목록과 **완전히 일치**한다.

따라서 다음 관계는 강하게 교차검증됐다.
...
다만 2024 HMI Application Name은 `GFENet`이므로, `GFENet`과 2026 DeviceXPlorer의 제품/런타임 관계는 별도 확인한다.

또한 2024 HMI에는 `P52`, `P56`, `PLC3`가 추가 존재한다. HMI Tag DB 대조 결과 P52는 P2/여과지 계열 417개 IOReal, P56는 P6/활성탄 계열 72개 IOReal로 묶여 있어 **보조 Analog Topic 성격이 강해졌다.** PLC3는 침전지 대표탁도 1 Point다. 별도 PLC 대수로 바로 해석하지 않는다.
...
4. 2024 `dde.cfg` 14,983개 GFENet Point ↔ XG5000 주소 대조
5. 현재 2026 DeviceXPlorer의 실제 DDE/SuiteLink Application Name
6. `GFENet ↔ DeviceXPlorer` 관계
7. Historian Tag 및 Source
8. Historian 실제 저장기간/주기/Quality
9. LINE A/B 설비별 이중화 방식 — `211=LINE A / 212=LINE B` 명칭은 확인됐으나 설비별 Master/Slave/단일PLC 듀얼NIC 구조 구분 필요
...
4. **HMI Tag Master + `dde.cfg` Point Mapping 활용** — 14,870개 Tag 메타데이터와 14,983개 GFENet Point를 PLC Address Dictionary와 대조
5. **현재 DXP Application 확인** — `GFENet ↔ DeviceXPlorer` 관계 확정
6. **History 설정 확보** — `dhistcfg.ini / historian.txt / Historian Tag Export`
7. **P8/P52/P56 역할 확정** — 2026 XGWX/DXP와 교차검증
8. **AI Canonical Mapping 작성** — 광암 정수 AI 학습항목으로 정규화
