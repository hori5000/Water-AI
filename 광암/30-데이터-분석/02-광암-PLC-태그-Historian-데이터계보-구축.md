...
## 2026-09-09 데이터계보 진행상태

기존 목표 중 `InTouch Tag ↔ DDE/Topic/Item` 연결은 현재 실제 완료됐다.

```text
HMI IO Tag = 15,147
DDE 전체 Point = 15,153
HMI IO Tag 15,147개 전부 DDE TagName 매칭
```

또 2022 Historian 2,235 Tag를 TagName으로 현재 HMI/DDE에 붙였다.
...
↔ 2026 현재 Historian Tag/메타데이터
↔ 실제 History Storage/실데이터
```

이다.

상세: [[05-광암-2026-HMI-DDE-Historian-통합매핑-결과]]


## 2026-09-09 Historian DB 분석 이후 데이터계보 상태
...
→ 2026 Runtime Tag/ItemName/StorageRate 메타데이터
→ [미확보] 실제 History Storage Timestamp/Value/Quality
```

최신 `backup2.bak` Runtime에서 확인된 대표 예:

| Historian Tag | 설명 | ItemName | StorageRate |
|---|---|---|---:|
| `AE_901` | 오존처리수 탁도 | `MW128/U` | 1000 |
| `AE_902A` | 활성탄 처리수 탁도 | `MW129/U` | 1000 |
| `AOP_INJQUAN_PV` | AOP 과산화수소 현재주입량(PV) | `MW2002` | 1000 |

따라서 **주소·태그 계보는 점점 완성되고 있으나 모델 학습에 필요한 실제 과거값은 아직 없다.**

병행 우선순위:

1. **데이터 확보**: History Storage/장기 Export
2. **구조 매핑**: `MW주소 ↔ XG5000 Symbol/Comment/Program Block`
3. History 데이터 확보 후 실제 값 시간대조로 `confirmed` 승격
