...
flowchart LR
    ...
    A2P -->|"XGT / FEnet"| SCADA
    SCADA --> DB["SQL/DB 계층<br/>실제 장기 Historian 위치 재확인 필요"]
    SCADA --> LOGGER["CIMON Logger<br/>AI / DI / DI2"]
    DB --> AI["AI Master DB / 분석"]
    LOGGER --> AI
```

> PLC 자체에는 과거이력 장기백업이 없다. 또한 9/7 PC의 XLOG 61개에서 `junglang` SQL 연결실패가 확인되었으므로, **실제 장기 Historian/DB가 어느 PC/서버에 있고 어떤 기간을 보존하는지 확인하는 것이 현재 최우선 병목**이다.
...
- **PLC 자체 과거이력 백업이 없고, 수집 HMI의 `junglang` SQL 연결실패도 확인되었으므로** 실제 Logger/DB 서버의 위치·저장주기·보존기간·백업정책·누락기간을 우선 확인해야 한다.
