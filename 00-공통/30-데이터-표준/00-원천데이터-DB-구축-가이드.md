...
## 4. 분석 작업공간과 공식 원천 DB를 구분한다

AI 개발자가 장기 Logger를 빠르게 분석하기 위한 **개발용 작업공간**과, 과제 기관들이 공동 참조할 **공식 원천/Canonical DB**는 같은 의사결정이 아니다.

| 구분 | 현재 권고 | 의미 |
|---|---|---|
| 원본 증거 | CLD/CSV/DB Export/Backup 불변 보존 | 재현·감사 원천 |
| Batch/Staging | Parquet | 대량 시계열 변환·교환 |
| 로컬 분석 | DuckDB + Python/Polars/PyArrow | Baseline·Dataset QA·Replay 개발 |
| 사람 검토 | Excel/CSV Export | 검토·보고 |
| 공식 원천/Canonical DB | A/B 설계안 + 4인 Gate 후 선택 | 운영·기관공유 기준선 |

> **중요:** DuckDB를 로컬 분석에 사용하는 것은 `waterai_*` 운영 DB나 공식 DB 엔진을 DuckDB로 확정한다는 뜻이 아니다. 공식 DB 선택현황은 [[90-현장별-설계안-선택현황]]에서만 관리한다.
